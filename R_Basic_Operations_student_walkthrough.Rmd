...
# This is a comment
#Hey comment
#To run a cell, you can press play or type ctrl enter
```
...
Integers must be whole numbers, if passed a floating value, the number will be rounded down to an integer.
...
```{r}
#commas tell R to put one variable after another and add a space
first_name <- "Joe"
last_name <- "Smith"
...
```{r}
#does not work like indexing. Starts at 1, not zero. includes last value
